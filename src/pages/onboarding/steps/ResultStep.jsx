...
const ResultStep = ({ result, onFinish }) => {
  const { placement, roadmap } = result;
  ...
  return (
    <div className="flex flex-1 flex-col gap-6">
      ...
      <div className="flex gap-3">
        <Metric label={uz.onboarding.resultStartingLevel} value={placement.startingLevel} accent />
        <Metric label={uz.onboarding.resultScore} value={`${placement.correct}/${placement.total}`} />
      </div>
                  ...
                  <span
                    className={`flex h-12 w-12 items-center justify-center rounded-2xl text-sm font-extrabold ${
                      isTarget
                        ...
                        : isStart
                          ? 'border border-accent/60 bg-accent-soft text-accent-bright'
